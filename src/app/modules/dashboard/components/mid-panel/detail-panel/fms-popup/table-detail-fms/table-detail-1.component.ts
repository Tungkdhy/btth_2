...
export class TableDetail1Component implements OnInit {
  ...
  async onPageChange(page: number) {
    this.page = page;
    let data: any;
    if (this.searchText != '' && this.capType != '' && this.xuLyType != '') {
      if (this.status) {
        data = await this.supabase.getDanhSachServerFmc(
          page,
          this.pageSize,
          this.status,
        );
      } else {
        data = await this.supabase.getDanhSachServerFmc(page, this.pageSize);
      }
  ...
  async search() {
    this.page = 1;
    let data = await this.supabase.getDanhSachServerFmc(
      1,
      5,
      this.status,
      this.searchText,
  ...
  async exportExcel() {
    try {
      ...
      const data_sheet = data.items.map((item: any) => ({
        'Tên đơn vị': `${item?.unit_name_manager_parent || ''} - ${
          item?.unit_name_manager || ''
        }`,
  ...
  async getLogStatus(id: string) {
    let res = await this.supabase.ptm_chi_tiet_khacfuc(
      id,
      null,
      null,
      null,
    ...
    if (Array.isArray(res)) {
      ...
      this.popUpDetail = res?.items || [];
    }
    console.log(this.popUpDetail);
    this.cdr.detectChanges();
    // Open the modal
  }
  showPopup(uuid: string) {
    this.getLogStatus(uuid);
