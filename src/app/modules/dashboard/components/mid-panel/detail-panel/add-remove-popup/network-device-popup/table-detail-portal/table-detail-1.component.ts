...
export class TableDetail1Component {
  ...
  async onPageChange(page: number) {
    this.page = page;
    let data = await this.supabase.getDanhSachTangGiamNetworkDevicePopupMidPanel(this.mainType,this.subType,this.typeAddRemove, this.convertService.getRegionType(this.regionType),this.searchText,this.is_deleted,this.source,this.startDate,this.endDate,page, this.pageSize);
  ...
  async search(){
    ...
    let data = await this.supabase.getDanhSachTangGiamNetworkDevicePopupMidPanel(this.mainType,this.subType,this.typeAddRemove, this.convertService.getRegionType(this.regionType),this.searchText,this.is_deleted,this.source,this.startDate,this.endDate,1, this.pageSize);
    this.dataSource = data?.items;
  ...
  async exportExcel() {
    let data = await this.supabase.getDanhSachTangGiamNetworkDevicePopupMidPanel(this.mainType,this.subType,this.typeAddRemove, this.convertService.getRegionType(this.regionType),this.searchText,this.is_deleted,this.source,this.startDate,this.endDate,1, 99999);
    // this.dataSource = data?.items;
    let data_sheet = [];
