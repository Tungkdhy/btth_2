...
import { TableDetail1Component } from "./table-detail-other-device/table-detail-1.component";
...
@Component({
  selector: 'app-other-device-popup',
  standalone: true,
  imports: [CommonModule, ColumnChartLeftPanelComponent, TableDetail1Component, StatisticPieChartPopupComponent],
  templateUrl: './other-device-popup.component.html',
  styleUrls: ['./other-device-popup.component.scss']
})
export class OtherDevicePopupComponent implements OnInit {
  constructor(
  ...
  async getData(){
    ...
    let thongKeEndPoint = await this.supabase.getThongKeBieuDoTronEndPointMidPanel(this.mainType,this.subType,"OTHER", this.convertService.getRegionType(this.regionType));
    ...
    let danhSachThongKeMatKetNoiThietBi:any = await this.supabase.getThongKeBieuDoCotEndPointPanel(this.mainType,this.subType,"OTHER",this.convertService.getRegionType(this.regionType));
    ...
    let tempTable :any = await this.supabase.getTableEndPoindMidPanel(this.mainType,this.subType,"OTHER",this.convertService.getRegionType(this.regionType));
