...
export class TableDetail1Component {
  ...
  async onPageChange(page: number) {
    this.page = page;
    let data = await this.supabase.getTableUDDVPopupMidPanel(this.mainType,this.subType,"COMMON",this.convertService.getRegionType(this.regionType),this.searchText,this.statusType,page,this.pageSize);
  ...
  async search(){
    ...
    let data = await this.supabase.getTableUDDVPopupMidPanel(this.mainType,this.subType,"COMMON",this.convertService.getRegionType(this.regionType),this.searchText,this.statusType,1,this.pageSize);
    this.dataSource = data?.items;
  ...
  async exportExcel() {
    let data = await this.supabase.getTableUDDVPopupMidPanel(this.mainType,this.subType,"COMMON",this.convertService.getRegionType(this.regionType),this.searchText,this.statusType,1,99999);
    // this.dataSource = data?.items;
    let data_sheet = [];
    ...
    for (let item of data.items) {
      data_sheet.push({
        ...
        Region: this.getRegionType(item?.loi),
    ...
    this.excelService.exportExcel(data_sheet, 'Dùng chung');
  }
