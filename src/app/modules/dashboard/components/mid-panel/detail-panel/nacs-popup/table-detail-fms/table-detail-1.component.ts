...
export class TableDetail1Component implements OnInit {
  ...
  async search() {
    this.page = 1;
    let data = await this.supabase.getDanhSachServerNACS(
      1,
      5,
      this.status,
      this.mainType,
  ...
  async onPageChange(page: number) {
    this.page = page;
    let data: any;
    if (this.searchText != '' && this.xuLyType != '') {
      if (this.status) {
        data = await this.supabase.getDanhSachServerNACS(
          page,
          this.pageSize,
          this.status,
          this.mainType,
  ...
  async exportExcel() {
    try {
      ...
      const data_sheet = data.items.map((item: any) => ({
        'Tên đơn vị': item?.unit_name_full?.replace('\n', '-') || '',
  ...
  async getLogStatus(id: string) {
    let res = await this.supabase.ptm_chi_tiet_khacfuc(id, null, null, 1, 1000);
