...
export class TableDetail1Component implements OnInit {
  ...
  async onPageChange(page: number) {
    this.page = page;
    let data = await this.supabase.getDanhSachThietBiPopupMidPanel(this.mainType,this.subType,"FIREWALL",this.convertService.getRegionType(this.regionType),this.searchText,this.statusType,this.source,null,this.page,this.pageSize);
  ...
  async search(){
    ...
    let data = await this.supabase.getDanhSachThietBiPopupMidPanel(this.mainType,this.subType,'FIREWALL',this.convertService.getRegionType(this.regionType),this.searchText,this.statusType,this.source,null,1,this.pageSize);
    this.dataSource = data?.items;
  ...
  async exportExcel() {
    let data = await this.supabase.getDanhSachThietBiPopupMidPanel(this.mainType,this.subType,'FIREWALL',this.convertService.getRegionType(this.regionType),this.searchText,this.statusType,this.source,null,1,9999);
    // this.dataSource = data?.items;
    let data_sheet = [];
    ...
    this.excelService.exportExcel(data_sheet, 'Tường lửa');
  }
