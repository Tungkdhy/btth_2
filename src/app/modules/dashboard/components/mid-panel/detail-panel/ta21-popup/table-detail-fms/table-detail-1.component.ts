...
export class TableDetail1Component implements OnInit {
  ...
  async onPageChange(page: number) {
    ...
    if (this.searchText != '' && this.capType != '' && this.xuLyType != '') {
      if (this.status) {
        data = await this.supabase.getDanhSachServerTA21(
          page,
          this.pageSize,
          this.status,
          this.ta21Data,
  ...
  async search() {
    this.page = 1;
    let data = await this.supabase.getDanhSachServerTA21(
      1,
      5,
      this.status,
      this.ta21Data,
  ...
  async exportExcel() {
    try {
      ...
      const data_sheet = data.items.map((item: any) => ({
        'Tên đơn vị': item?.unit_name_full || '',
        'Thời gian kết nối': this.getLastActive(item?.last_up),
        Vùng: this.getType(item?.unit?.region),
