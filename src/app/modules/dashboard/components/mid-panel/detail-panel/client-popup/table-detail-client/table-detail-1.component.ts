...
export class TableDetail1Component {
  ...
  async onPageChange(page: number) {
    this.page = page;
    let data = await this.supabase.getTableEndPoindMidPanel(
      this.mainType,
      this.subType,
      'CLIENT',
  ...
  async search(){
    ...
    let data = await this.supabase.getTableEndPoindMidPanel(this.mainType,this.subType,"CLIENT",this.convertService.getRegionType(this.regionType),[this.selectedValues], this.searchText,this.statusType,1,this.pageSize);
    this.dataSource = data?.items;
    this.page =  data?.page_index;
    this.pageSize = data?.page_size;
    this.total =  data?.total;
  ...
  async exportExcel() {
    let data = await this.supabase.getTableEndPoindMidPanel(this.mainType,this.subType,"CLIENT",this.convertService.getRegionType(this.regionType),[this.selectedValues], this.searchText,this.statusType,1,99999);
    // this.dataSource = data?.items;
    let data_sheet = [];
    ...
    this.excelService.exportExcel(data_sheet, 'Máy trạm');
  }
  getSources(sources: []) {
