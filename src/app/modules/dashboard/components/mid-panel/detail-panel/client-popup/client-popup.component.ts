...
import { TableDetail1Component } from "./table-detail-client/table-detail-1.component";
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ColumnChartLeftPanelComponent, TableDetail1Component, UploadPostBarComponent, StatisticPieChartPopupComponent],
...
export class ClientPopupComponent implements OnInit {
  ...
  async getData(){
    ...
    let thongKeEndPoint = await this.supabase.getThongKeBieuDoTronEndPointMidPanel(this.mainType,this.subType,"CLIENT",this.convertService.getRegionType(this.regionType));

    this.pieData = [
      { 
        name: 'Đang hoạt động', 
        value: thongKeEndPoint?.find((e:any)=>e?.not_online == false)?.count  || 0,
        itemStyle: { color: 'rgba(4, 94, 43, 1)' } 
      },
      { 
        name: 'Không hoạt động', 
        value: thongKeEndPoint?.find((e:any)=>e?.not_online == true)?.count  || 0,
        itemStyle: { color: 'rgba(242, 159, 6, 1)' }
      },
    ];

    let columnData = await this.supabase.getBieuDoBarEndPointDevice(this.mainType,this.subType,this.convertService.getRegionType(this.regionType),"CLIENT");

    let osData = columnData?.find((e:any)=>e.name =="os")?.groupx || [];
    ...
    let tempTable : any = await this.supabase.getTableEndPoindMidPanel(this.mainType,this.subType,"CLIENT",this.convertService.getRegionType(this.regionType));
